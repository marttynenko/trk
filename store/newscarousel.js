...
export const actions = {
  async fetchPosts ({ commit}) {
    try {
      ...
      const news = await this.$axios.$get(`${config.APIserver}/api/element/?filter[iblock_id]=${iblockID}&filter[news_slider_value]=${encodeURIComponent('Да')}&filter[active]=Y&filter[active_date]=Y&sort=active_from:desc&fields=id,name,active_from,code&limit=12`)
