...
export const actions = {
  async fetchPosts ({ commit}) {
    try {
      ...
      const news = await this.$axios.$get(`${config.APIserver}/api/element/?filter[iblock_id]=${iblockID}&&filte[active]=Y&filter[actual_value]=${encodeURIComponent('Да')}&sort=active_from:desc&fields=id,name,active_from,code&limit=6?clear=Y`)
