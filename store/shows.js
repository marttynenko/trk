...
export const mutations = {
  ...
  updateFrontCategories(state,arr) {
    ...
    const clean = list.map(el => {
      ...
      modifiered.ID = el.ID
      modifiered.NAME = el.NAME
      // modifiered.DESCRIPTION = el.DESCRIPTION.replace(/<\/?[^>]+>/ig, " ").substring(0,320) + '...'
      modifiered.DESCRIPTION = el.DESCRIPTION.replace(/<\/?[^>]+>/ig, " ").substring(0,320)
      if (modifiered.DESCRIPTION) {modifiered.DESCRIPTION += '...'} 
      modifiered.IMG = (el.DETAIL_PICTURE && el.DETAIL_PICTURE.SRC)
...
export const actions = {
  ...
  async fetchFrontCategories({commit}) {
    try {
      ...
      const cats = await this.$axios.$get(`${config.APIserver}/api/section/?filter[iblock_id]=${iblockID}&filter[active]=Y&sort=sort:desc&fields=id,name,code,detail_picture,description&limit=5`)
  ...
  async fetchShowsBlock ({ commit}) {
    try {
      ...
      const news = await this.$axios.$get(`${config.APIserver}/api/element/?filter[iblock_id]=${iblockID}&filter[active]=Y&filter[iblock_section.active]=Y&sort=active_from:desc&fields=video_link,active_from,name&limit=4`)
